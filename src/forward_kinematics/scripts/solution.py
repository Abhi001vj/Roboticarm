...
class ForwardKinematics(object):
    ...
    def compute_transforms(self, link_names, joints, joint_values):
        ...
        lt = []
        joints1 = joints[1:]
        link_names1 = link_names[1:]
        print link_names
        print joint_values
        t0 = tf.transformations.concatenate_matrices(
        ...
        lt.append(convert_to_message(t0,link_names[0],"world_link"))
        for i,j in enumerate(joints1):
            #print joints1[i].name
            na = j.name
            
            #[i],joint_values[0]
            ij = joint_values.name.index(na)
            jp = joint_values.position[ij]
            #print na ,ij, jp
        # YOUR CODE GOES HERE
            #print type(j.origin.rpy[0])
            #print type(jp)
            r, p, y = j.origin.rpy
            t1 = tf.transformations.concatenate_matrices(
            
            tf.transformations.translation_matrix(j.origin.xyz),
            tf.transformations.quaternion_matrix(
            tf.transformations.quaternion_from_euler(r, p, y)),
            tf.transformations.quaternion_matrix(
            tf.transformations.quaternion_about_axis(jp, j.axis))
            )
            ...
            t2 = numpy.dot(t0,t1)
            t = convert_to_message(t2,link_names1[i],"world_link")
            lt.append(t)
            t0 = t2        
        ...
        lt.append(convert_to_message(t3,link_names[6],"world_link"))
        all_transforms.transforms = lt
